...
/**
 * Save or update SPTA to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving SPTA:', data.title);
    ...
    console.log('✓ SPTA saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving SPTA to Supabase:', error.message);
    throw error;
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ SPTA deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting SPTA from Supabase:', error.message);
    return false;
  }
}
