// ==================== INDEXEDDB SETUP FOR MOOE ====================
const DB_NAME = 'WBSSDatabase_MOOE';
const DB_VERSION = 2;
const STORE_NAME = 'mooe';
const FILE_STORE_NAME = 'mooeFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `mooe/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${MOOE_BUCKET}/`);
...
/**
 * Save or update MOOE to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving MOOE:', data.title);
    ...
    console.log('✓ MOOE saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving MOOE to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all MOOE documents from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading MOOE documents from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `mooe-${memo.id}.pdf`,
    ...
    console.log('Migrated MOOE file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ MOOE deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting MOOE from Supabase:', error.message);
    return false;
  }
}
