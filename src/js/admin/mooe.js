...
/**
 * Save or update MOOE module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving MOOE module:', data.title);
    ...
    console.log('✓ MOOE module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving MOOE module to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all MOOE from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading MOOE module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ MOOE module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting MOOE module from Supabase:', error.message);
    return false;
  }
}
