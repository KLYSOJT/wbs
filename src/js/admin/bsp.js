// ==================== INDEXEDDB SETUP FOR BSP ====================
const DB_NAME = 'WBSSDatabase_BSP';
const DB_VERSION = 2;
const STORE_NAME = 'bsp';
const FILE_STORE_NAME = 'bspFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `bsp/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${BSP_BUCKET}/`);
...
/**
 * Save or update BSP to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving BSP:', data.title);
    ...
    console.log('✓ BSP saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving BSP to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all BSP documents from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading BSP documents from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `bsp-${memo.id}.pdf`,
    ...
    console.log('Migrated BSP file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ BSP deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting BSP from Supabase:', error.message);
    return false;
  }
}
