// ==================== INDEXEDDB SETUP FOR TR ====================
const DB_NAME = 'WBSSDatabase_TR';
const DB_VERSION = 2;
const STORE_NAME = 'tr';
const FILE_STORE_NAME = 'trFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `tr/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${TR_BUCKET}/`);
...
/**
 * Save or update TR to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving TR:', data.title);
    ...
    console.log('✓ TR saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving TR to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all TR documents from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading TR documents from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `tr-${memo.id}.pdf`,
    ...
    console.log('Migrated TR file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ TR deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting TR from Supabase:', error.message);
    return false;
  }
}
