...
/**
 * Save or update TR module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving TR module:', data.title);
    ...
    console.log('✓ TR module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving TR module to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all TR from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading TR module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ TR module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting TR module from Supabase:', error.message);
    return false;
  }
}
