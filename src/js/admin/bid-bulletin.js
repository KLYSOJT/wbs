// ==================== INDEXEDDB SETUP FOR BID BULLETIN ====================
const DB_NAME = 'WBSSDatabase_BidBulletin';
const DB_VERSION = 2;
const STORE_NAME = 'bidBulletin';
const FILE_STORE_NAME = 'bidBulletinFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `bid-bulletin/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${BID_BULLETIN_BUCKET}/`);
...
/**
 * Save or update Bid Bulletin to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving Bid Bulletin:', data.title);
    ...
    console.log('✓ Bid Bulletin saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving Bid Bulletin to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all Bid Bulletins from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading Bid Bulletins from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `bid-bulletin-${memo.id}.pdf`,
    ...
    console.log('Migrated Bid Bulletin file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
/**
 * Delete Bid Bulletin from Supabase
 */
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ Bid Bulletin deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting Bid Bulletin from Supabase:', error.message);
    return false;
  }
}
