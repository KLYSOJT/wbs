...
const DB_VERSION = 1;
const STORE_NAME = 'bid_bulletin';
const FILE_STORE_NAME = 'bid_bulletinFiles';
...
/**
 * Save or update Bid Bulletin module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving Bid Bulletin module:', data.title);
    ...
    console.log('✓ Bid Bulletin module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving Bid Bulletin module to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all Bid Bulletin from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading Bid Bulletin module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ Bid Bulletin module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting Bid Bulletin module from Supabase:', error.message);
    return false;
  }
}
