// ==================== INDEXEDDB SETUP FOR GSP ====================
const DB_NAME = 'WBSSDatabase_GSP';
const DB_VERSION = 2;
const STORE_NAME = 'gsp';
const FILE_STORE_NAME = 'gspFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `gsp/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${GSP_BUCKET}/`);
...
/**
 * Save or update GSP to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving GSP:', data.title);
    ...
    console.log('✓ GSP saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving GSP to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all GSP documents from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading GSP documents from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `gsp-${memo.id}.pdf`,
    ...
    console.log('Migrated GSP file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ GSP deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting GSP from Supabase:', error.message);
    return false;
  }
}
