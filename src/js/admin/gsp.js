...
/**
 * Save or update GSP module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving GSP module:', data.title);
    ...
    console.log('✓ GSP module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving GSP module to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all GSP from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading GSP module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ GSP module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting GSP module from Supabase:', error.message);
    return false;
  }
}
