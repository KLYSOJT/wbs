// ==================== INDEXEDDB SETUP FOR SSLG ====================
const DB_NAME = 'WBSSDatabase_SSLG';
const DB_VERSION = 2;
const STORE_NAME = 'sslg';
const FILE_STORE_NAME = 'sslgFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `sslg/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${SSLG_BUCKET}/`);
...
/**
 * Save or update SSLG to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving SSLG:', data.title);
    ...
    console.log('✓ SSLG saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving SSLG to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all SSLG documents from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading SSLG documents from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `sslg-${memo.id}.pdf`,
    ...
    console.log('Migrated SSLG file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ SSLG deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting SSLG from Supabase:', error.message);
    return false;
  }
}
