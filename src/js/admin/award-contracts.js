// ==================== INDEXEDDB SETUP FOR AWARD_CONTRACTS ====================
const DB_NAME = 'WBSSDatabase_AwardContracts';
const DB_VERSION = 1;
const STORE_NAME = 'award_contracts';
const FILE_STORE_NAME = 'award_contractsFiles';
...
/**
 * Save or update Award Contracts module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving Award Contracts module:', data.title);
    ...
    console.log('✓ Award Contracts module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving Award Contracts module to Supabase:', error.message);
    throw error;
  }
}
...
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading Award Contracts module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ Award Contracts module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting Award Contracts module from Supabase:', error.message);
    return false;
  }
}
