// ==================== INDEXEDDB SETUP FOR AWARD CONTRACTS ====================
const DB_NAME = 'WBSSDatabase_AwardContracts';
const DB_VERSION = 2;
const STORE_NAME = 'awardContracts';
const FILE_STORE_NAME = 'awardContractsFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `award-contracts/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${AWARD_CONTRACTS_BUCKET}/`);
...
/**
 * Save or update Award Contracts to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving Award Contracts:', data.title);
    ...
    console.log('✓ Award Contracts saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving Award Contracts to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all Award Contracts from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading Award Contracts from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `award-contracts-${memo.id}.pdf`,
    ...
    console.log('Migrated Award Contracts file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
/**
 * Delete Award Contracts from Supabase
 */
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ Award Contracts deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting Award Contracts from Supabase:', error.message);
    return false;
  }
}
