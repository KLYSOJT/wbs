...
/**
 * Save or update APP module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving APP module:', data.title);
    ...
    console.log('✓ APP module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving APP module to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all APP from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading APP module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ APP module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting APP module from Supabase:', error.message);
    return false;
  }
}
