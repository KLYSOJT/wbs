// ==================== INDEXEDDB SETUP FOR APP ====================
const DB_NAME = 'WBSSDatabase_APP';
const DB_VERSION = 2;
const STORE_NAME = 'app';
const FILE_STORE_NAME = 'appFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `app/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${APP_BUCKET}/`);
...
/**
 * Save or update APP to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving APP:', data.title);
    ...
    console.log('✓ APP saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving APP to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all APPs from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading APPs from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `app-${memo.id}.pdf`,
    ...
    console.log('Migrated APP file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ APP deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting APP from Supabase:', error.message);
    return false;
  }
}
