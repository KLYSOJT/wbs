// ==================== INDEXEDDB SETUP FOR DEPED MEMORANDUM ====================
const DB_NAME = 'WBSSDatabase_DepedMemo';
const DB_VERSION = 2;
const STORE_NAME = 'depedMemo';
const FILE_STORE_NAME = 'depedMemoFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `deped-memo/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${DEPED_MEMO_BUCKET}/`);
...
/**
 * Save or update DepEd Memorandum to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving DepEd Memorandum:', data.title);
    ...
    console.log('✓ DepEd Memorandum saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving DepEd Memorandum to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all DepEd Memorandums from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading DepEd Memorandums from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `deped-memo-${memo.id}.pdf`,
    ...
    console.log('Migrated DepEd Memorandum file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
/**
 * Delete DepEd Memorandum from Supabase
 */
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ DepEd Memorandum deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting DepEd Memorandum from Supabase:', error.message);
    return false;
  }
}
