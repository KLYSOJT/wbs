...
/**
 * Save or update PhilGEPS module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving PhilGEPS module:', data.title);
    ...
    console.log('✓ PhilGEPS module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving PhilGEPS module to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all PhilGEPS from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading PhilGEPS module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ PhilGEPS module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting PhilGEPS module from Supabase:', error.message);
    return false;
  }
}
