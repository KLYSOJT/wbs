// ==================== INDEXEDDB SETUP FOR PHILGEPS ====================
const DB_NAME = 'WBSSDatabase_PhilGEPS';
const DB_VERSION = 2;
const STORE_NAME = 'philgeps';
const FILE_STORE_NAME = 'philgepsFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `philgeps/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${PHILGEPS_BUCKET}/`);
...
/**
 * Save or update PhilGEPS to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving PhilGEPS:', data.title);
    ...
    console.log('✓ PhilGEPS saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving PhilGEPS to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all PhilGEPSs from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading PhilGEPSs from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `philgeps-${memo.id}.pdf`,
    ...
    console.log('Migrated PhilGEPS file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ PhilGEPS deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting PhilGEPS from Supabase:', error.message);
    return false;
  }
}
