    ...
    if (window.supabaseClient?.storage) {
      try {
        ...
        if (sessionError || !sessionData?.session) {
          throw new Error('Please log in as an admin before uploading DepEd Order files.');
        }
        ...
        if (/row-level security|unauthorized|jwt|not authenticated/i.test(storageError.message || '')) {
          throw new Error('Supabase Storage blocked the upload. Apply the deped-order-files storage migration/policies, then try again.');
        }
...
/**
 * Save or update DepEd Order to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving DepEd Order:', data.title);
    ...
    console.log('✓ DepEd Order saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving DepEd Order to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all DepEd Orders from Supabase
 */
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    console.warn(`Skipping migration for DepEd Order ${memo.id}:`, error.message);
    return memo;
  }
}
...
/**
 * Delete DepEd Order from Supabase
 */
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ DepEd Order deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting DepEd Order from Supabase:', error.message);
    return false;
  }
}
      ...
      if (memo.file.startsWith('idb://')) {
        ...
        fileDisplay = `
          <button
            ...
            onclick="downloadIdbFile(${memo.id}, '${escapedTitle}')"
        ...
        fileDisplay = `
          <a
            href="${escapeHtml(memo.file)}"
            target="_blank"
            rel="noopener noreferrer"
            class="action-icon-btn action-icon-btn--view"
            aria-label="View ${escapeHtml(memo.title)}"
            title="View document">
            <i class="fas fa-eye" aria-hidden="true"></i>
          </a>
        `;
        ...
        <td>
          <div class="action-icon-group">
            <button
              ...
              aria-label="Edit ${escapeHtml(memo.title)}"
              title="Edit document">
            ...
            <button
              ...
              aria-label="Delete ${escapeHtml(memo.title)}"
...
function waitForAdminAuthReady(timeoutMs = 10000) {
  if (!window.adminAuth) {
    return Promise.resolve();
  }

  if (typeof window.adminAuth.isAuthenticated === 'function' && window.adminAuth.isAuthenticated()) {
    return Promise.resolve();
  }

  return new Promise((resolve, reject) => {
    const timeoutId = window.setTimeout(() => {
      window.removeEventListener('adminAuthVerified', handleVerified);
      reject(new Error('Admin authentication was not verified before loading DepEd Orders.'));
    }, timeoutMs);
    ...
    window.addEventListener('adminAuthVerified', handleVerified, { once: true });
  });
}
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    ...
    console.error('DepEd Order initialization error:', error);
  }
