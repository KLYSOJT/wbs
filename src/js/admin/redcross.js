// ==================== INDEXEDDB SETUP FOR RED CROSS ====================
const DB_NAME = 'WBSSDatabase_RedCross';
const DB_VERSION = 2;
const STORE_NAME = 'redcross';
const FILE_STORE_NAME = 'redcrossFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `redcross/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${REDCROSS_BUCKET}/`);
...
/**
 * Save or update Red Cross to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving Red Cross:', data.title);
    ...
    console.log('✓ Red Cross saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving Red Cross to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all Red Cross documents from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading Red Cross documents from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `redcross-${memo.id}.pdf`,
    ...
    console.log('Migrated Red Cross file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
/**
 * Delete Red Cross from Supabase
 */
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ Red Cross deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting Red Cross from Supabase:', error.message);
    return false;
  }
}
