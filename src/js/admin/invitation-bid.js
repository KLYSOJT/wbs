// ==================== INDEXEDDB SETUP FOR INVITATION TO BID ====================
const DB_NAME = 'WBSSDatabase_InvitationBid';
const DB_VERSION = 2;
const STORE_NAME = 'invitationBid';
const FILE_STORE_NAME = 'invitationBidFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `invitation-bid/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${INVITATION_BID_BUCKET}/`);
...
/**
 * Save or update Invitation to Bid to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving Invitation to Bid:', data.title);
    ...
    console.log('✓ Invitation to Bid saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving Invitation to Bid to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all Invitation to Bids from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading Invitation to Bids from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `invitation-bid-${memo.id}.pdf`,
    ...
    console.log('Migrated Invitation to Bid file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
/**
 * Delete Invitation to Bid from Supabase
 */
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ Invitation to Bid deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting Invitation to Bid from Supabase:', error.message);
    return false;
  }
}
