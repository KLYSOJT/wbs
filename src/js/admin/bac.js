// ==================== INDEXEDDB SETUP FOR BAC ====================
const DB_NAME = 'WBSSDatabase_BAC';
const DB_VERSION = 2;
const STORE_NAME = 'bac';
const FILE_STORE_NAME = 'bacFiles';
...
function getStorageFilePath(file, memoId) {
  ...
  return `bac/${memoId}-${Date.now()}-${safeName}`;
}
  ...
  try {
    ...
    if (fileUrl.startsWith('http')) {
      try {
        ...
        const urlParts = fileUrl.split(`/${BAC_BUCKET}/`);
...
/**
 * Save or update BAC to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving BAC:', data.title);
    ...
    console.log('✓ BAC saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving BAC to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all BACs from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading BACs from Supabase:', error.message);
    return [];
  }
}
...
async function migrateMemoFileToPublicUrl(memo) {
  ...
  try {
    ...
    const uploadSource = new File(
      [fileData.blob],
      fileData.fileName || `bac-${memo.id}.pdf`,
    ...
    console.log('Migrated BAC file to public URL:', memo.id);
    return migratedMemo;
  } catch (error) {
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ BAC deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting BAC from Supabase:', error.message);
    return false;
  }
}
