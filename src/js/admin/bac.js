...
/**
 * Save or update BAC module to Supabase (with file)
 */
async function saveMemoToSupabase(data) {
  try {
    ...
    console.log('Saving BAC module:', data.title);
    ...
    console.log('✓ BAC module saved to Supabase:', data.title);
    return result ? result[0] : data;
  } catch (error) {
    console.error('Error saving BAC module to Supabase:', error.message);
    throw error;
  }
}

/**
 * Load all BAC from Supabase
 */
async function loadMemosFromSupabase() {
  try {
    ...
    console.error('Error loading BAC module from Supabase:', error.message);
    return [];
  }
}
...
async function deleteMemoFromSupabase(id, fileUrl) {
  try {
    ...
    console.log('✓ BAC module deleted from Supabase');
    return true;
  } catch (error) {
    console.error('Error deleting BAC module from Supabase:', error.message);
    return false;
  }
}
