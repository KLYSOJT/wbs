...
function renderPagination() {
  ...
  if (depedOrderState.currentPage > 1) {
    fragment.appendChild(createPaginationButton('Previous', depedOrderState.currentPage - 1));
  }

  const startPage = Math.max(1, depedOrderState.currentPage - 2);
  const endPage = Math.min(totalPages, startPage + 4);

  for (let page = startPage; page <= endPage; page += 1) {
    fragment.appendChild(createPaginationButton(String(page), page, page === depedOrderState.currentPage));
  }

  if (depedOrderState.currentPage < totalPages) {
    fragment.appendChild(createPaginationButton('Next', depedOrderState.currentPage + 1));
  }
