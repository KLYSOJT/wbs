...
function renderPagination() {
  ...
  fragment.appendChild(
    createPaginationButton('Previous', appState.currentPage - 1, {
      isDisabled: appState.currentPage === 1,
      extraClass: 'pagination-btn--nav',
      ariaLabel: 'Previous page'
    })
  );

  buildPaginationItems(totalPages, appState.currentPage).forEach((item) => {
    if (item === 'ellipsis') {
    ...
    fragment.appendChild(
      createPaginationButton(String(item), item, {
        isActive: item === appState.currentPage,
        ariaLabel: 'Page ' + item
      })
    );
  });
  ...
  fragment.appendChild(
    createPaginationButton('Next', appState.currentPage + 1, {
      isDisabled: appState.currentPage === totalPages,
      extraClass: 'pagination-btn--nav',
      ariaLabel: 'Next page'
    })
  );
