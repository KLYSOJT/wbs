...
function renderPagination() {
  ...
  fragment.appendChild(
    createPaginationButton('Previous', sptaState.currentPage - 1, {
      isDisabled: sptaState.currentPage === 1,
      extraClass: 'pagination-btn--nav',
      ariaLabel: 'Previous page'
    })
  );

  buildPaginationItems(totalPages, sptaState.currentPage).forEach((item) => {
    if (item === 'ellipsis') {
    ...
    fragment.appendChild(
      createPaginationButton(String(item), item, {
        isActive: item === sptaState.currentPage,
        ariaLabel: 'Page ' + item
      })
    );
  });
  ...
  fragment.appendChild(
    createPaginationButton('Next', sptaState.currentPage + 1, {
      isDisabled: sptaState.currentPage === totalPages,
      extraClass: 'pagination-btn--nav',
      ariaLabel: 'Next page'
    })
  );
