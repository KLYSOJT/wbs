...
function renderPagination() {
  ...
  fragment.appendChild(
    createPaginationButton('Previous', gspState.currentPage - 1, {
      isDisabled: gspState.currentPage === 1,
      extraClass: 'pagination-btn--nav',
      ariaLabel: 'Previous page'
    })
  );

  buildPaginationItems(totalPages, gspState.currentPage).forEach((item) => {
    if (item === 'ellipsis') {
    ...
    fragment.appendChild(
      createPaginationButton(String(item), item, {
        isActive: item === gspState.currentPage,
        ariaLabel: 'Page ' + item
      })
    );
  });
  ...
  fragment.appendChild(
    createPaginationButton('Next', gspState.currentPage + 1, {
      isDisabled: gspState.currentPage === totalPages,
      extraClass: 'pagination-btn--nav',
      ariaLabel: 'Next page'
    })
  );
